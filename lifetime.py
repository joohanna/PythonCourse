...
#Description: This python script calculates the lifetime of a bridging water contacts between  protein residues and a dna residues.
...
parser.add_argument("files", nargs="*", type=str, help="The name of the input files.")
parser.add_argument("--dtime", "-dt", type=int, help="time interval in ps between each frame in the contact files. Default is 1 ps", default=1)
parser.add_argument("--lib", "-l", type=str, help="residue libary file to name the contacts (optional)", default="NONE")
parser.add_argument("--out", "-o", type=str, help="output name of the file", default="allLifetimes.txt")
...
class Lifetime:
	...
	#contactName: name of the contact "ProtRes-WAT-DNAres/filename" (str)
...
class Contacts:
	...
	def NewLifetimeObject(self):
		#method for creating objects to the class Lifetime for bridning water contacts stored in args.files.
		if args.lib != "NONE":
...
def calcLifetime(list):
	...
	for line in list:
		...
		if line == 1:
			...
			lifetime = lifetime*args.dtime #Multiply lifetime with arg.dtime to get the lifetime in ps
